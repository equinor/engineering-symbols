...
export abstract class WorldObject {
  ...
  private moveOnGrab(w: Readonly<World>): void {
    ...
    this.isGrabbed =
      this.isSelected && this.isGrabStarted && w.mouse.isLeftButtonDown;
  ...
  private handleLeftButtonClicked(w: Readonly<World>, posClicked: Vec2): void {
    ...
    const anotherObjInSamePos =
      w.selectedWorldObjects.find(
        (c) => c.posFrame.equals(this.posFrame) && c.key !== this.key
      ) !== undefined;
...
function drawSelectionBox(
  ...
  wRect: WorldObjectRect,
  color: string
) {
  const ph = getSelectionBorderPattern(wRect.width);
  const pv =
    wRect.height === wRect.width
      ? [...ph]
      : getSelectionBorderPattern(wRect.height);
