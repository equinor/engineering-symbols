...
import {
  ...
  SelectedObjects,
  SymbolEditorEvent,
} from "./models/EditorEvent";
import { generateRandomString } from "./utils/strings";
import { EditorCommandMessage } from "./models/EditorCommand";
import {
  InternalEditorEventHandlers,
  addWorldEventListeners,
  createWorldEventHandlers,
  removeWorldEventListeners,
} from "./event";
...
export class World {
  ...
  private loadSymbol(symbolDto: SymbolData) {
    this.symbol = {
      ...
      centerOfRotation: new Vec2(
        symbolDto.centerOfRotation.x,
        symbolDto.centerOfRotation.y
      ),
  ...
  private calculateUserInput() {
    ...
    if (this.panActive) {
      this.frame.center = this.frame.center.add(
        this.mouse.pos.sub(this.mouse.posLast)
      );
    }
    ...
    if (this.symbol) {
      ...
      this.frame.origin = this.frame.center.sub(
        this.frame.screenSize.scale(0.5)
      );

      this.mouse.posFrame = this.mouse.pos
        .sub(this.frame.origin)
