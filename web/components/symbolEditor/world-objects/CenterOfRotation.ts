import { World } from '../World';
import { Point } from '../models/SymbolData';
import { Vec2 } from '../models/Vec2';
import { WorldObject, WorldObjectRect } from '../models/WorldObject';
import { resetScale } from '../utils/context2d';

// 24x24
const corGeo =
	'M9,12c0,1.66,1.34,3,3,3s3-1.34,3-3s-1.34-3-3-3S9,10.34,9,12z M8,9L8,9c0-0.55-0.45-1-1-1H5.09C6.47,5.61,9.05,4,12,4c3.49,0,6.45,2.24,7.54,5.36C19.68,9.75,20.07,10,20.48,10h0 c0.68,0,1.18-0.67,0.96-1.31C20.07,4.79,16.36,2,12,2C8.73,2,5.82,3.58,4,6.01V5c0-0.55-0.45-1-1-1h0C2.45,4,2,4.45,2,5v4 c0,0.55,0.45,1,1,1h4C7.55,10,8,9.55,8,9z M16,15L16,15c0,0.55,0.45,1,1,1h1.91c-1.38,2.39-3.96,4-6.91,4c-3.49,0-6.45-2.24-7.54-5.36C4.32,14.25,3.93,14,3.52,14h0 c-0.68,0-1.18,0.67-0.96,1.31C3.93,19.21,7.64,22,12,22c3.27,0,6.18-1.58,8-4.01V19c0,0.55,0.45,1,1,1h0c0.55,0,1-0.45,1-1v-4 c0-0.55-0.45-1-1-1h-4C16.45,14,16,14.45,16,15z';

const corH = 24;
const corW = 24;
const corPathScale = 1.5;

export class CenterOfRotation extends WorldObject {
	posFrame: Vec2;
	r = 6;
	corPath = new Path2D(corGeo);

	constructor() {
		super('CenterOfRotation');
		this.hidden = true;
		this.type = 'CenterOfRotation';
		this.posFrame = new Vec2(0, 0);
		this.isSelectable = true;
		this.isDraggable = true;
	}

	isInsideHitBox(pos: Vec2): boolean {
		return pos.distance(this.posClient) < corH / 2;
	}

	toExternalModel(): Point {
		return { x: this.posFrame.x, y: this.posFrame.y };
	}

	getBoundingBoxRect(): WorldObjectRect {
		const size = this.r * 2 + 25;
		return {
			origin: this.posClient.sub(new Vec2(size / 2, size / 2)),
			width: size,
			height: size,
		};
	}

	onUpdate(w: Readonly<World>): void {
		this.hidden = !w.symbol;

		if (!w.symbol) {
			return;
		}

		if (w.events.symbol.load) {
			this.posFrame = w.symbol.centerOfRotation.clone();
		}

		if (this.wasDragged) {
			w.symbol.centerOfRotation = this.posFrame.clone();
			w.notifyCenterOfRotationUpdated();
		}
	}

	onDraw(ctx: CanvasRenderingContext2D): void {
		if (this.hidden) return;

		ctx.beginPath();
		ctx.arc(this.posClient.x, this.posClient.y, this.r, 0, 2 * Math.PI);
		ctx.fillStyle = '#075985';
		ctx.fill();
		resetScale(ctx);

		ctx.translate(this.posClient.x - (corPathScale * corW) / 2, this.posClient.y - (corPathScale * corH) / 2);
		ctx.scale(corPathScale, corPathScale);
		ctx.globalAlpha = 1.0;
		ctx.fillStyle = '#075985';
		ctx.fill(this.corPath);
		resetScale(ctx);
	}
}
